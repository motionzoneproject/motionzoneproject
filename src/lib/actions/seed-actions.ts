...
export async function seedNewProduct(
...
export async function seedAddCourseToProduct(
  ...
  try {
    ...
    const isInProd = await seedIsCourseInProduct(
      formData.courseId,
      formData.productId,
    );

    if (isInProd.found) {
      const added = await prisma.productOnCourse.update({
        where: {
          courseId_productId: {
            courseId: validated.courseId,
      ...
      const added = await prisma.productOnCourse.create({
        data: {
          productId: validated.productId,
          courseId: validated.courseId,
          lessonsIncluded: validated.lessonsIncluded,
...
export async function seedIsCourseInProduct(
  courseId: string,
  productId: string,
